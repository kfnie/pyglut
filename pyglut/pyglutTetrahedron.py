...
from polyhedrons_generators import generate_tetrahedron
...
class Tetrahedron(object) :
  def __init__(self,side_length,display_mode="lined",lines_color=False,faces_color=False,lines_width=1,display_ls=False) :
    ''' Generate an Tetrahedron object with the given side length settings.
        ...
        faces_color  = An objet <type 'Color'> representing the faces color.
                       An 4-items-list from object <type 'Color'>.
                       One item per tetrahedron face. 
    ...
    if faces_color :
      ...
      elif isinstance(faces_color,list) and len(faces_color) != 4 :
        print("Error faces_color argument:\nYou must give an list from 4 Color objects.\nOne Color object per face.")
        quit()
      elif isinstance(faces_color,list) and len(faces_color) == 4 :
        tmp=[]
        faces_color_index=0
        while faces_color_index < 4 :
    ...
    self.polyhedron=generate_tetrahedron(side_length)
    self.ls=Localview()
  ...
  def __doc__(self) :
    ...
    print('''
    Generate an tetrahedron object with position updating, 
    display and settings setters methods.
    
    The display mode set how the tetrahedron will be display 
    ...
    -> An object from <type 'Color'> in which case the tetrahedron 
